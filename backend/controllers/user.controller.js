...
export const followUnfollowUser = async (req, res) => {
  ...
  try {
    ...
    if (isFollowing) {
      ...
      res.status(200).json({ message: "User unfollowed succefully" });
    } else {
      // Follow the user
...
export const updateUser = async (req, res) => {
  ...
  try {
    ...
    if(profileImg) {
        if (user.profileImg) {
            // https://res.cloudinary.com/dyfqon1v6/image/upload/v1712997552/zmxorcxexpdbh8r0bkjb.png I need last part only
            await cloudinary.v2.uploader.destroy(user.profileImg.split("/").pop().split(".")[0]);
        }

        const uploadedResponse = await cloudinary.v2.uploader.upload(profileImg)
    ...
    if(coverImg) {
        ...
        const uploadedResponse = await cloudinary.v2.uploader.upload(coverImg)
